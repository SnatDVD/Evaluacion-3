...
public class Hospital {
    ...
    private HashMap<Medico,ArrayList<Paciente>> listado; 
...
// ------------ Añado los enfermeros al hospital
    ...
    public void agregarPersona(Persona persona) {
        ...
        if(persona instanceof Medico){
            listado.put((Medico) persona, new ArrayList());
        } else if (persona instanceof Paciente){
            Medico aux = ((Paciente) persona).getDoctor();
            if(listado.containsKey(aux)){
                listado.get(aux).add((Paciente)persona);
            }
        }
    }
    ...
    public int numPacientesDoctor(Medico medico){
        if(listado.containsKey(medico)){
            return listado.get(medico).size();
        } else {
            return 0;
        }
    }
    ...
    private char letraDni(String dni){
        ...
        int aux = Integer.valueOf(dni) % 23;
        return letras[aux];
    }
